...
function enableEdit() {
    ...
    const userServices = document.querySelectorAll('#services-requested-collection li')
    const servicesList = document.querySelectorAll('#services-requested-collection label')
    // Collect all Platforms 
    const userPlatform = document.querySelectorAll('#platform-requested-collection li')
    const platformList = document.querySelectorAll('#platform-requested-collection label')

    userServices.forEach(function (li) {

        servicesList.forEach(function (services) {

            if (li.textContent.trim() == services.textContent.trim()) {
                console.log(services.textContent);
                console.log(services.closest('div').querySelector('input').checked = true);
                services.closest('div').querySelector('input').checked = true;
                services.closest('div').querySelector('input').setAttribute("checked", 'checked');
                console.log(services.closest('div').querySelector('input'));

            }
        })
    })

    userPlatform.forEach(function (li) {
        platformList.forEach(function (services) {
            if (li.textContent.trim() == services.textContent.trim()) {
                services.closest('div').querySelector('input').checked = true;
                services.closest('div').querySelector('input').setAttribute("checked", 'checked');
...
//  ----------- Dcline Request -----------  //
const declineBtn = document.querySelector('#form-decline-btn')
declineBtn && declineBtn.addEventListener('click', declineRequest);
console.log('s');
function declineRequest() {
    ...
    Swal.fire({
        ...
        <div class="form-group">
            <label for="exampleFormControlTextarea1 p-2 ">Provide reason for declining</label>
        ...
        preConfirm: () => {
            ...
            document.getElementById("declineDetails").value = modalDetail
            Swal.fire({
                title: "Processing your Request",
                // html: "I will close in <b></b> milliseconds.",
                timer: 3000,
...
//  ----------- Force complete Request Output-----------  //
const forceCompleteBtn = document.querySelector('#form-force-complete-btn')
forceCompleteBtn && forceCompleteBtn.addEventListener('click', forceComplete);
console.log('s');
function forceComplete() {
    ...
    Swal.fire({
        html: `
        <div class="form-group">
            <label for="exampleFormControlTextarea1 p-2 ">Provide reason for </label>
            <textarea class="form-control rounded-input" id="swal-detail" rows="5" name="additional-info"></textarea>
        </div>
        `,
        confirmButtonColor: '#084F08',
        ...
        preConfirm: () => {
            modalDetail = document.getElementById("swal-detail").value,
            document.getElementById("forceCompleteDetails").value = modalDetail

            Swal.fire({
                html: `
                <div class="form-group">
                    <label for="exampleFormControlTextarea1 p-2 ">Input Admins Password To confirm </label>
                    <input type="text" id="swal-password">
                </div>
                `,
                confirmButtonColor: '#084F08',
                focusConfirm: false,
                ...
                allowEnterKey: true,
                preConfirm: () => {
                    modalDetail = document.getElementById("swal-password").value,
                    document.getElementById("adminPassword").value = modalDetail
...
//  ----------- Add-Output Request -----------  //
const requestContribution = document.querySelector('#add-output-btn')
const requestOutput = document.querySelector('.requestOutput')
requestContribution && requestContribution.addEventListener('click', addOutput)
function addOutput() {
    Swal.fire({
        title: "Add Output",
        html: `
...
function updateRequest() {
    ...
    var input1 = document.querySelector('input[name="activity-name"]');
    var input2 = document.querySelector('input[name="activity-Duration-viewpage"]');
    var input3 = document.querySelector('input[name="activity-venue"]');
    var input4 = document.querySelector('input[name="activity-participants"]');
    var input5 = document.querySelector('input[name="activity-officials"]');
    var input6 = document.querySelector('input[name="activity-highlights"]');

    var input7 = document.querySelectorAll('input[name="services[]"]:checked');
    var valuesServices = [];
    input7.forEach(function (checkbox) {
        valuesServices.push(checkbox.value);
    });

    console.log(form);
    var input8 = document.querySelectorAll('input[name="platforms[]"]:checked');
    var valuesPlatforms = [];
    input8.forEach(function (checkbox) {
        valuesPlatforms.push(checkbox.value);
    });

    var input9 = document.querySelector('textarea[name="additional-info"]');
    var input10 = document.querySelector('input[name="attachement[]"]');



    form.appendChild(input1.cloneNode(true));
    form.appendChild(input2.cloneNode(true));
    form.appendChild(input3.cloneNode(true));
    form.appendChild(input4.cloneNode(true));
    form.appendChild(input5.cloneNode(true));
    form.appendChild(input6.cloneNode(true));
    form.appendChild(input9.cloneNode(true));
    form.appendChild(input10.cloneNode(true));
    // Concatenate selected services into a single hidden input

    var servicesInput = document.createElement('input');
    servicesInput.type = 'hidden';
    servicesInput.name = 'services[]';
    servicesInput.value = JSON.stringify(valuesServices);
