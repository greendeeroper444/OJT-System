...
const activityInformationContentInputs = document.querySelector('.activity-information-handler textarea')
...
requestSubmitBtn.onclick = (event) => {
    ...
    activityInformationInputs.forEach((input) => {
        ...
        if (input.id == "others-input" || input.id == "attachement" || input.id == "post-link") {
            return
        }
        console.log('ss');
        if (input.value == '') {
    ...
    if (activityInformationContentInputs.value == '') {
        noError = false
        activityInformationContentInputs.classList.add('rounded-input-error')
        activityInformationContentInputs.closest('.form-group').querySelector('label').classList.add('name-activity-error')
        activityInformationContentInputs.closest('.card').querySelector('div').style.backgroundColor = "#B20404";
    }
